...
from anhc_agent_interfaces.srv import Step, Reset, Seed, GetDimensions, SampleActionSpace
from ament_index_python.packages import get_package_share_directory

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SOURCE_UTILS_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'utils'))
INSTALL_UTILS_DIR = os.path.join(
    get_package_share_directory('anhc_agent'),
    'scripts',
    'utils',
)
for utils_dir in (SOURCE_UTILS_DIR, INSTALL_UTILS_DIR):
...
class Environment(Node):
    ...
    def __init__(self):
        ...
        env_config_file_path = os.path.join(
            anhc_agent_src_path, "anhc_agent", "config", env_config_file_name
        )
        start_goal_pairs_file_path = os.path.join(
            anhc_agent_src_path, "anhc_agent", "config", start_goal_pairs_file
        )
    ...
    def step_callback(self, request, response):
        ...
        # Capture current distance BEFORE overwriting agent_state slots
        ...
        reward = self.get_reward(
            target, collision, action, min_laser,
            self.prev_distance_to_goal, current_distance
        )

        # Update prev_distance for the next step
        self.prev_distance_to_goal = current_distance
    ...
    def reset_callback(self, _, response):
		...
		*****************************************************"""
        ...
        # Initialize prev_distance for the new episode
    ...
    @staticmethod
    def get_reward(target, collision, action, min_laser, prev_distance, curr_distance):
        """Calculate reward with distance-based shaping.

        Components:
        - distance_reward: (prev - curr) * 2.0 → positive when approaching goal
        - obstacle_reward: penalty for being close to obstacles (min_laser < 1.0)
        - action_reward:   encourage forward motion, penalize excessive turning
        """
        if target:
            return 100.0
        if collision:
            return -100.0
        # Distance-based reward: approaching goal = positive, moving away = negative
        distance_reward = (prev_distance - curr_distance) * 2.0
        obstacle_reward = (min_laser - 1) / 2 if min_laser < 1.0 else 0.0
        action_reward = action[0] / 2 - abs(action[1]) / 2 - 0.001
        return distance_reward + obstacle_reward + action_reward

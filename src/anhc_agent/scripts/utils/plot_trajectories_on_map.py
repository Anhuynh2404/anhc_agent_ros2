...
def plot_comparison(
    ...
    map_path = os.path.join(anhc_agent_src_path, "anhc_agent", "maps")
    test_runs_path = os.path.join(anhc_agent_src_path, "anhc_agent", "test_runs")
    start_goal_pairs_path = os.path.join(anhc_agent_src_path, "anhc_agent", "config")
...
def main():
    anhc_agent_src_env = "ANHC_AGENT_SRC_PATH"
    anhc_agent_src_path = os.getenv(anhc_agent_src_env)
    if anhc_agent_src_path is None:
        print(f"Environment variable: {anhc_agent_src_env}, is not set")
        sys.exit(-1)

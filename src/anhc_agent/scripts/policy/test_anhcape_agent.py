...
class TestAnhcape(EnvInterface):
    def __init__(self):
        super().__init__("test_anhcape_agent")
		...
		************************************************"""
        ...
        test_config_file_path = os.path.join(
            anhc_agent_pkg_path, "config", "test_config.yaml"
        )
        self.pytorch_models_dir = os.path.join(
            anhc_agent_pkg_path, "temp", "pytorch_models"
        )
        ...
        self.test_metric_dir = os.path.join(anhc_agent_pkg_path, "test_runs")
...
def main():
    ...
    test_anhcape = TestAnhcape()
    executor = MultiThreadedExecutor(num_threads=2)
    executor.add_node(test_anhcape)
    try:
        while rclpy.ok():
            executor.spin()
        ...
        test_anhcape.get_logger().info(f"{'Testing is Done':-^35}")
        test_anhcape.get_logger().info("rclpy, shutting down...")
        test_anhcape.destroy_node()
        rclpy.shutdown()
